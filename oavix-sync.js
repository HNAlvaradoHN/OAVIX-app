...
(function(){
  ...
  const session = () => { try { return JSON.parse(nativeGet(SESSION_KEY) || 'null'); } catch (e) { console.warn('[OAVIX] Sesión local ilegible, se ignora.', e); return null; } };
  ...
  function accountSnapshot(email){ try{return JSON.parse(nativeGet(metaKey(email))||'null');}catch(e){console.warn('[OAVIX] Copia local de la cuenta ilegible, se ignora.',e);return null;} }
  function saveAccountSnapshot(email, updatedAt){ if(!email)return null; const snap={schemaVersion:5,updatedAt:updatedAt||new Date().toISOString(),data:dataSnapshot()}; try{ nativeSet(metaKey(email),JSON.stringify(snap)); }catch(e){ console.error('[OAVIX] No se pudo guardar la copia local de la cuenta.',e); toast('⚠ No se pudo guardar','El almacenamiento del navegador está lleno o bloqueado; los cambios podrían perderse.','rose'); return null; } return snap; }
  function restoreAccount(email){ const snap=accountSnapshot(email); if(!snap)return false; DATA_KEYS.forEach(k=>{ if(Object.prototype.hasOwnProperty.call(snap.data||{},k)) nativeSet(k,snap.data[k]); else nativeRemove(k); }); nativeSet(localUpdatedKey(email),snap.updatedAt||new Date().toISOString()); nativeSet(LAST_SYNC_KEY,snap.updatedAt||''); nativeRemove(PENDING_KEY); return true; }
  ...
  window.addEventListener('unhandledrejection',e=>{console.error('[OAVIX] Promesa rechazada sin manejar:',e.reason);});
  window.addEventListener('online',()=>{if(accountEmail&&nativeGet(PENDING_KEY)==='true')setTimeout(()=>{syncNow(false).catch(e=>console.error('[OAVIX] Sincronización automática fallida.',e));},400);});
  window.addEventListener('beforeunload',()=>{if(accountEmail)try{saveAccountSnapshot(accountEmail,nativeGet(localUpdatedKey(accountEmail))||(accountSnapshot(accountEmail)||{}).updatedAt||new Date().toISOString());}catch(e){console.error('[OAVIX] No se pudo guardar la copia local al salir.',e);}});
  document.addEventListener('DOMContentLoaded',()=>{setTimeout(()=>{try{initUI();}catch(e){console.error('[OAVIX] Falló la inicialización de la interfaz de sincronización.',e);toast('⚠ Error de interfaz','No se pudo preparar el panel de sincronización.','rose');}},0);if(accountEmail&&navigator.onLine&&nativeGet(PENDING_KEY)==='true')setTimeout(()=>{syncNow(false).catch(e=>console.error('[OAVIX] Sincronización inicial fallida.',e));},900);},{once:true});
})();
...
(function(){
  ...
  function fixCategories(){
    let stored = null;
    try { stored = JSON.parse(localStorage.getItem(CATEGORY_KEY)); }
    catch (e) { console.warn('[OAVIX] Categorías guardadas ilegibles, se restauran las predeterminadas.', e); stored = null; }
