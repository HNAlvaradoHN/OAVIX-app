...
(function(){
  ...
  async function syncNow(interactive){
    ...
    try{
      ...
      /* El motivo real (permiso denegado, Client ID ausente, error de Drive) debe
         llegar al usuario: antes cualquier fallo se anunciaba como simple falta de red. */
      toast('⚠ Guardado localmente','No se pudo sincronizar: '+((e&&e.message)||'error desconocido')+'. Los datos siguen guardados en este dispositivo.','amber');
    }finally{busy=false;}
  }
  ...
  document.addEventListener('DOMContentLoaded',()=>{setTimeout(()=>{try{initUI();}catch(e){console.error('[OAVIX] Falló la inicialización de la interfaz de sincronización.',e);toast('⚠ Error de interfaz','No se pudo preparar el panel de sincronización.','rose');}},0);if(accountEmail&&navigator.onLine)setTimeout(()=>{syncNow(false).catch(e=>console.error('[OAVIX] Sincronización inicial fallida.',e));},900);},{once:true});
})();
